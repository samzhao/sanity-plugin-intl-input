...
export const PublishWithi18nAction = (props: IResolverProps) => {
  ...
  const { publish } = useDocumentOperation(
    props.id,
    props.type
  ...
  return {
    ...
    label: publishing ? config.messages?.publishing : config.messages?.publish,
    onHandle: async () => {
      setPublishing(true);
      ...
      const languageId =
        getLanguageFromId(props.id) ||
        getBaseLanguage(langs, config.base)?.name;
      ...
      await client
        .patch(props.draft?._id || props.id, {
          set: { [fieldName]: languageId },
        })
        .commit();
      publish.execute();

      const translatedDocuments = await client.fetch<SanityDocument[]>(
        "*[_id match path($id)]",
        {
          id: buildDocId(baseDocumentId, "**"),
        }
      );
